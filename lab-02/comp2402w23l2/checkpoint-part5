...
public class Part5 {
    ...
    public static void execute(BufferedReader r, PrintWriter w) throws IOException, NumberFormatException {
        ...
        ArrayStack<ArrayStack<Integer>> groups = new ArrayStack<>();
        ArrayStack<Integer> group = new ArrayStack<>();
        int index = 0;
        int size = 1;
        ...
        for (int i = 0; i < numbers.n; i++) {
            if (i == index + size - 1) {
                for (int j = index; j < index + size; j++)
                    group.add(0, numbers.get(j));
                groups.add(group);
                group = new ArrayStack<>();
                index = i+1;
                size++;
            }
            if (i == numbers.n - 1 && (numbers.n - index) > 0) {
                for (int j = index; j < numbers.n; j++)
                    group.add(0, numbers.get(j));
                groups.add(group);
                break;
            }
        ...
        ArrayStack<Integer> ordered_numbers = new ArrayStack<>();
        for (ArrayStack<Integer> inner : groups) {
            for (int element : inner) ordered_numbers.add(element);
        }

        ArrayStack<Integer> last_group = groups.get(groups.n - 1);
        int sum = 0;
        for (int i = 0; i < last_group.n; i++) {
            int index_to_add = (last_group.get(i) + numbers.n) % numbers.n;
            sum = (sum % 240223) + (ordered_numbers.get(index_to_add)) % 240223;
        }
