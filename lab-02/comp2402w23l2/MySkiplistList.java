...
public class MySkiplistList<T> implements MyList<T> {
    ...
    public void copy(int k) {
        ...
        int index = 1;
        // pointers to traverse the skip list
        Node ptr = sentinel.next[0];
        Node temp = ptr;

        // traverse the skiplist (by setting `ptr` = next)
        while(ptr != null) {
            ptr = ptr.next[0];
            // for each iteration, add temp's date to index `index`
            for(int j = 1; j < k; j++) add(index, temp.x);
            // increment pointers to keep traversing list
            temp = ptr;
            index += k;
        } return;
    }
