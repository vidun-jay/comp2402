 ...
 *      % java comp2402w23l5.Part1 <x-value> <input.txt> <output.txt>
 *  will take input from <input.txt> and will print to <output.txt>, or
 *      % java comp2402w23l5.Part1 <x-value> <input.txt>
 *  will take input from <input.txt> and will print to the terminal, or
 *      % java comp2402w23l5.Part1  <x-value>
 ...
 *      % python3 run_local_tests.py student_config_lab5p1.json
...
public class Part1 {
    ...
    public static void execute(int x, BufferedReader r, PrintWriter w) throws IOException, NumberFormatException {
        ...
        java.util.ArrayDeque<Integer> numbers = new java.util.ArrayDeque<>();
        ...
        while ((line = r.readLine()) != null) {
            ...
            if (curr % prev == 0) {
                numbers.offer(curr);
                if (numbers.size() > x) numbers.pollFirst();
            } prev = curr;
        }

        while (!numbers.isEmpty()) w.println(numbers.pollLast());
    }
