 ...
 *      % java comp2402w23l5.Part2 <x-value> <input.txt> <output.txt>
 *  will take input from <input.txt> and will print to <output.txt>, or
 *      % java comp2402w23l5.Part2 <x-value> <input.txt>
 *  will take input from <input.txt> and will print to the terminal, or
 *      % java comp2402w23l5.Part2 <x-value>
 ...
 *      % python3 run_local_tests.py student_config_lab5p2.json
    ...
    public static void execute(int x, BufferedReader r, PrintWriter w) throws IOException, NumberFormatException {
        ...
        while ((line = r.readLine()) != null) {
            ...
            if (curr % prev == 0) {
                numbers.add(curr);
                if (numbers.size() > x)
                    numbers.remove(numbers.iterator().next());
            } prev = curr;
        }

        ArrayStack<Integer> reversed = new ArrayStack<>();
        reversed.addAll(numbers);
        while (!reversed.isEmpty()) w.println(reversed.remove(0));
    }
