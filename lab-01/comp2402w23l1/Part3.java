...
public class Part3 {
    ...
    public static void execute(BufferedReader r, PrintWriter w) throws IOException, NumberFormatException {
        ...
        // if current line is divisible by previous, push to stack
        while ((line = r.readLine()) != null) {
            int curr = Integer.parseInt(line);
            if (curr % prev == 0)
                numbers.addLast(curr);
            prev = curr;
        } r.close();

        // pop the stack and print the result (resulting in reverse order)
        while (!numbers.isEmpty())
            w.println(numbers.removeLast());
